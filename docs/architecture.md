...
The maps deliberately distinguish implemented components (`implemented`, `mock-first`) from production work (`planned`). Supabase Auth, snapshot/RLS, and consent-gated private media are implemented; external AI and App Store billing remain planned.
...
3. Move analysis and generation calls behind server-side functions. Keep provider keys and job orchestration out of the Expo bundle; preserve the existing `queued → processing → completed|failed` lifecycle.
